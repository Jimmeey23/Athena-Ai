...
function formatTicketBody(value: string): string {
  ...
  return lines.map((line) => {
    ...
    if (/^(member|client|guest|prospect|host|trainer)\b/i.test(line)) return `- ${line}`;
    return line;
  }).join('\n');
}
...
export const TicketProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const escalatedTicketIdsRef = useRef<Set<string>>(new Set());
  ...
  const managerScope = useMemo(() => {
    const managedEmployees = ASSOCIATES.filter((employee) => (
      visibleIdentityValues.has(String(employee.manager || '').trim().toLowerCase())
    ));
    const departmentValues = new Set(
      ASSOCIATES
        .filter((employee) => (
          visibleIdentityValues.has(employee.name.trim().toLowerCase()) ||
          visibleIdentityValues.has(String(employee.email || '').trim().toLowerCase())
        ))
        .map((employee) => employee.team.trim().toLowerCase())
        .filter(Boolean)
    );

    for (const employee of managedEmployees) {
      if (employee.team) departmentValues.add(employee.team.trim().toLowerCase());
    }

    return {
      teamMemberIdentityValues: new Set(
        managedEmployees
          .flatMap((employee) => [employee.name, employee.email])
          .filter(Boolean)
          .map((value) => String(value).trim().toLowerCase())
      ),
      departmentValues,
  ...
  const canSeeTicket = useCallback((ticket: Ticket) => {
    return canAccessTicketForIdentity({
      ...
      teamMemberIdentityValues: managerScope.teamMemberIdentityValues,
      departmentValues: managerScope.departmentValues,
      ticket,
    });
  }, [accessRole, managerScope, visibleIdentityValues]);
  ...
  useEffect(() => {
    ...
    readNotificationIdsRef.current = localIds;
    ...
    backendSupabase
      ...
      .then(({ data, error: dismissalError }) => {
        ...
        const merged = mergeDismissedNotificationIds(readNotificationIdsRef.current, remoteIds);
        readNotificationIdsRef.current = merged;
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === 'INSERT') {
            ...
            setSelectedTicketState((prev) => (prev && prev.id === id ? null : prev));
          }
        }
      )
      .subscribe();
  ...
  const updateTicket = useCallback(
    async (id: string, patch: Partial<Ticket>, actor = getReporterNameFromAuthUser(user), options: { bypassAuthz?: boolean } = {}) => {
      const rowPatch = toRowPatch(patch);

      // Get current ticket to compute event diff
      const current = tickets.find((t) => t.id === id);

      // Historic rows have no DB record; apply locally only so every touch
      // doesn't fail against a non-existent row and trigger a full refresh.
      ...
      if (!options.bypassAuthz && current) {
        const privilegedFields = ['status', 'assignedTo', 'priority', 'title', 'description', 'metadata', 'team'];
        const touchesPrivileged = privilegedFields.some((field) => field in patch);
        if (touchesPrivileged && !canUpdateTicketStatus(current)) {
          throw new Error('Only the assigned ticket owner or their immediate supervisor can edit this ticket.');
        }
      }
      ...
      // Log events for important changes; isolated so a log failure never
      // fails an update that actually persisted.
      if (current) {
        ...
        if (events.length > 0) {
          try {
            await backendSupabase.from('ticket_events').insert(events);
          } catch (eventError) {
            console.warn('Ticket event log insert failed:', getErrorMessage(eventError, 'Event log insert failed'));
          }
        }
      }
    },
  ...
  const updateTicketStatus = useCallback(
    async (id: string, detail: TicketStatusUpdateInput, actor = getReporterNameFromAuthUser(user)) => {
      ...
      if (!canUpdateTicketStatus(current)) {
        throw new Error('Only the assigned ticket owner or their immediate supervisor can change this ticket status.');
      }
  ...
  const updateTicketResolutionPlan = useCallback(
    async (id: string, plan: TicketResolutionPlan, actor = getReporterNameFromAuthUser(user)) => {
      ...
      if (!canEditTicketResolution(current)) {
        throw new Error('Only the assigned ticket owner or their immediate supervisor can edit the resolution plan.');
      }
  ...
  const deleteTicket = useCallback(
    async (id: string) => {
      const current = tickets.find((ticket) => ticket.id === id);
      if (current && !current.tags.includes('historic') && !canUpdateTicketStatus(current)) {
        throw new Error('Only the assigned ticket owner or their immediate supervisor can delete this ticket.');
      }
  ...
  const createApprovedTicket = useCallback(
    async (draft: DraftTicket, conversationId?: string | null, context?: Record<string, unknown>, attachments: File[] = []) => {
      ...
      if (relatedTickets.exactDuplicate) {
        ...
        await updateTicket(relatedTickets.exactDuplicate.id, duplicatePatch as Partial<Ticket>, signedInReporter, { bypassAuthz: true });
        const mergedTicket = {
          ...relatedTickets.exactDuplicate,
          ...duplicatePatch,
      ...
      const row: DbTicketPatch = {
        ...toInsertRow(draftForServer, contextForServer as Record<string, unknown>, publishAssignment as ResolvedAssignment),
        created_by: authData.session?.user.id,
      };
      ...
      for (let attempt = 0; attempt < 3; attempt += 1) {
        const { data: insertData, error: insertError } = await backendSupabase
          .from('tickets')
          .insert(rowForInsert)
          .select('*')
          .single();
        ...
        if (!insertError) {
          created = insertData as DbTicketRow;
          createError = null;
          break;
        }

        createError = insertError;
        const missingColumn = getMissingColumnName(insertError);
        if (!missingColumn || !(missingColumn in rowForInsert)) break;
        rowForInsert = removeUnsupportedTicketColumn(rowForInsert, missingColumn);
      }

      if (createError || !created) throwSupabaseError(createError, 'Ticket creation failed');
  ...
  useEffect(() => {
    ...
    for (const ticket of breached) {
      ...
      // Guard against unbounded retry: if the DB write persistently fails the
      // refresh() inside updateTicket re-renders `tickets` and re-runs this
      // effect. Each ticket is only ever escalated once per session.
      if (escalatedTicketIdsRef.current.has(ticket.id)) continue;
      escalatedTicketIdsRef.current.add(ticket.id);
      ...
      if (ticket.tags.includes('historic')) {
        ...
        updateTicket(ticket.id, patch, 'SLA Automation', { bypassAuthz: true }).catch((error) => {
          console.warn('SLA auto-escalation failed:', getErrorMessage(error, 'SLA escalation failed'));
        });
      }
    }
