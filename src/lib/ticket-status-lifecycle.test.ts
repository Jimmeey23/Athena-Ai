...
import { PRIORITY_SLA, type Ticket } from './ticketing-data';
...
describe('ticket status lifecycle', () => {
  it('keeps priority SLA definitions aligned to operational response windows', () => {
    expect(PRIORITY_SLA.Critical.hours).toBe(2);
    expect(PRIORITY_SLA.High.hours).toBe(8);
    expect(PRIORITY_SLA.Medium.hours).toBe(24);
    expect(PRIORITY_SLA.Low.hours).toBe(72);
  });
